...
CREATE TABLE IF NOT EXISTS habits (
  ...
  target_days INTEGER DEFAULT 1, -- For weekly/monthly: how many days per period
...
-- Track logs table (completion status per day)
...
CREATE TABLE IF NOT EXISTS ai_insights (
  ...
  insight_type VARCHAR(50) NOT NULL, -- 'pattern', 'suggestion', 'prediction', 'explanation'
...
CREATE TABLE IF NOT EXISTS notifications (
  id SERIAL PRIMARY KEY,
  user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  type VARCHAR(50) NOT NULL, -- 'reminder', 'streak_alert', 'encouragement'
